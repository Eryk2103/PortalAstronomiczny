...
@Component({
    selector: 'app-add-sub-comment',
    templateUrl: './add-sub-comment.component.html',
    styleUrls: ['./add-sub-comment.component.css']
})
export class AddSubCommentComponent implements OnInit {
    id: number;
    commentId: number
    form: FormGroup;
    comment: PostComment;
    ...
    onSubmit(){
        ...
        comment.likes = 0;
        comment.dislikes = 0;
        this.postService.postPostComment(comment)
        .subscribe(result => {
            this.router.navigate(['/post/'+this.id+"/comments"])
